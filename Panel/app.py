...
def calculate():
    ...
    irradiancia = float(request.args.get("irradiancia", 0))

    # Crear un objeto de la clase PVModel
    pv = PVModel(4, 3)

    # Calculo del modelo PV
...
def index():
    ...
    # Calculo del modelo PV
    resultados, Vmpp, Impp, P_max = pv.modelo_pv(G=1000, T=298)

    # return {f"Vmp = {Vmpp:.2f} V, Imp = {Impp:.2f} A, Pmax = {P_max:.2f} W"}
    return render_template('index.html', Vmpp=Vmpp, Impp=Impp, P_max=P_max)
